...
const { send, cancel, assign } = actions;
...
const konamiCodeMachine = Machine(
    ...
    states: {
      acceptingKeys: {
        ...
        on: {
          KEYUP: {
            target: "keyEntered",
            actions: ["assignKey", "sendDelayedReset"]
          }
        }
      },
      keyEntered: {
  ...
  },
  {
    ...
    guards: {
      isCodeComplete: (context) =>
        context.keys.join(" ") === "up up down down left right left right B A"
    }
  }
);
